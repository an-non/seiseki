...
# validation.mjs
path = Path("cloudflare/src/validation.mjs")
text = path.read_text(encoding="utf-8")
if "export function normalizeExpectedRevision" not in text:
...
'''
    if marker not in text:
        raise RuntimeError("validation insertion marker not found")
    text = text.replace(marker, addition + marker, 1)
path.write_text(text, encoding="utf-8")

# db.mjs
path = Path("cloudflare/src/db.mjs")
text = path.read_text(encoding="utf-8")
if "export async function getResponseQuestionSnapshot" not in text:
...
'''
    if marker not in text:
        raise RuntimeError("db insertion marker not found")
    text = text.replace(marker, addition + marker, 1)
path.write_text(text, encoding="utf-8")

# index.mjs
path = Path("cloudflare/src/index.mjs")
text = path.read_text(encoding="utf-8")
...
'''  deleteResponse,
  ...
  updateResponseFreeText''', "db imports")
text = replace_once(text,
...
'''import {
  ...
  normalizeSubmission,
  RequestError
} from "./validation.mjs";''', "validation imports")
text = replace_once(text,
...
if "function routeFreeTextId" not in text:
...
'''
    if marker not in text:
        raise RuntimeError("route marker not found")
    text = text.replace(marker, addition + marker, 1)

if "async function enqueueAnalysisRevision" not in text:
...
'''
    if marker not in text:
        raise RuntimeError("enqueue marker not found")
    text = text.replace(marker, addition + marker, 1)

if "const freeTextId = routeFreeTextId" not in text:
    marker = "  const analysisId = routeAnalysisId(url.pathname);"
    routes = r'''  const freeTextId = routeFreeTextId(url.pathname);
...
'''
    if marker not in text:
        raise RuntimeError("response route marker not found")
    text = text.replace(marker, routes + marker, 1)
path.write_text(text, encoding="utf-8")

# tests
path = Path("cloudflare/tests/response-phase4-backend.test.mjs")
...
print("Applied Phase 4 backend update API and recovery boundaries.")
